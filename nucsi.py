...
def setup_directories(output_dir):
    """Create necessary directories for the NuCSI pipeline."""
    dirs = {
...
def create_configs_yaml(output_dir, quality_cutoff):
    ...
    config_content = f"""# NuCSI Configuration File
# ========================
...
def create_makefile(output_dir):
    """Create Makefile for the NuCSI pipeline."""
...
def run_pipeline(output_dir):
    """Run the complete NuCSI pipeline."""
    print("\n" + "="*60)
    print("Running NuCSI Pipeline")
    print("="*60)
    ...
    try:
        ...
        if return_code == 0:
            print("\nNuCSI pipeline completed successfully!")
...
def main():
    parser = argparse.ArgumentParser(
        description="NuCSI: Nuclease Cleavage Site Identification Tool",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  nucsi.py -f sample_R1.fastq.gz sample_R2.fastq.gz -p plasmid.fasta -o results/
  nucsi.py -f sample1_R1.fastq.gz sample1_R2.fastq.gz sample2_R1.fastq.gz sample2_R2.fastq.gz -p plasmid.fasta -o results/ -q 30
  nucsi.py -f sample_R1.fastq.gz sample_R2.fastq.gz -p plasmid.fasta -o results/ --run-pipeline

For more information, visit: https://github.com/Matt115A/NuCSI
        """
    )
